...
Drupal.behaviors.activeMenu = function (context) {
  ...
  for (var menu in menus) {
    ...
    $(menu + ' li.collapsed:not(.activemenu-processed)').each(function() {
      ...
      var url = activeMenuUpdatedURL(this, Drupal.url(menus[menu]));
      
      // @todo: add support for menu_firstchild module.
...
function activeMenuUpdatedURL(liElement, url) {
  var linkClass = $('a:first', liElement).attr('class');
  
  if (linkClass) {
    var regex = /menu-([0-9]+)/;
    var mlid = regex.exec(linkClass);
    if (mlid) {
      url += '/'+ mlid[1];
    }
  }
